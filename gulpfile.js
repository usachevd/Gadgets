...
const entryPoints = fs.readdirSync('src/client/js', { withFileTypes: true })
    .filter(item => path.extname(item.name).toLowerCase() === '.js')
    .map(item => 'src/client/js/' + item.name);
...
const dev = series(setPorts, parallel(tsClient, html, views, images, bundleJS, bundleServer, css, data, publicUpdate), startServer, watchFiles);
